...
4주 프로젝트라도 — 아니, 오히려 4주이기 때문에 — 한 서비스의 빌드 실패가 다른 서비스의 데모를 막아서는 안 됩니다. 주차 발표 직전 learning-svc CI가 깨졌다고 platform/knowledge 데모까지 멈출 수는 없습니다.
...
4주 프로젝트의 가장 큰 위험은 "merge conflict 해결로 시간을 다 쓰는 것"입니다. 한 모노레포에 6명이 모이면 매일 충돌이 발생합니다. 폴리레포는 도메인별로 작업 영역을 물리적으로 분리해 충돌을 최소화합니다.
...
**결론**: 폴리레포의 단점은 도구로 해결 가능, 모노레포의 단점(도메인 결합 + 충돌 폭증)은 4주 안에 해결 불가. 그래서 폴리레포입니다.
...
각 주차 종료 직전(주차별 데모 D-1). 한 주 동안 4개 백엔드 + 프론트가 각자 배포한 새 버전을 검증해서 **주차별 통합 릴리즈 태그**를 찍습니다. 4주 프로젝트 기준 총 4회의 통합 릴리즈가 발생합니다.

| 주차 | 통합 릴리즈 태그 | 데모 범위 |
|:----:|:--------------:|---------|
| 1주차 말 | `v0.1.0` (alpha) | platform 인증 + 기본 frontend |
| 2주차 말 | `v0.2.0` (alpha) | + knowledge (노트/그래프) |
| 3주차 말 | `v0.3.0` (beta) | + engagement (커뮤니티/알림) |
