...
| **`@learning-card-owner`** | synapse-learning-svc / learning-card 골격 + card 모듈 (덱/카드 CRUD + SM-2 알고리즘 기초) |
| **`@learning-ai-owner`** | synapse-learning-svc / learning-ai 골격 (FastAPI scaffolding + Anthropic API 연동 + Embedding API 연결) |
...
| **`@learning-card-owner`** | synapse-learning-svc / learning-card (card + srs, Java) | learning-card 골격 + 덱/카드 CRUD + SM-2 | SRS 복습 세션 + Kafka 발행 | card.review.due 발행 + 복습 통계 | (W3 잔무 + 복습 전체 E2E) | 복습 안정화 + Kafka 안정화 | 발표 시연 |
| **`@learning-ai-owner`** | synapse-learning-svc / learning-ai (ai, Python/FastAPI) | learning-ai 골격 + LLM API 연결 | 시맨틱 검색 + AI 카드 골격 | AI 자동 생성 안정화 + 시맨틱 캐시 | RAG (시간 허용 시) + AI E2E + 정확도 검증 | 시맨틱 검색 안정화 | 발표 시연 |
...
## 5. 5주 MVP와 제품 장기 Phase 구분

본 17은 학기 프로젝트의 **5주 단기 일정**(W1~W5: 2026-05-12 ~ 06-12) + 발표일(2026-06-15)을 다룬다. 본 문서의 주차별 W1~W5는 **학기 프로젝트 5주 MVP 구현·시연 범위**이며, 제품화 이후의 장기 Phase와 구분한다.

| 구분 | 의미 | 범위 |
|------|------|------|
| **학기 프로젝트 5주 MVP** | 2026-05-12 ~ 2026-06-12 구현·시연 범위 | Auth/JWT/MFA 기초, Note CRUD, Card/SRS, Graph/Search, AI 카드 생성, Billing Test Mode, Notification, Community/Gamification 핵심, Admin/신고 핵심, Staging/E2E/발표 준비 |
| **제품 장기 Phase** | 5주 이후 제품화·확장 로드맵 | 운영 자동화, 고도화 기능, 대규모 트래픽 대응, 서비스 분리 검토 |

제품 장기 Phase는 다음 기준으로 관리한다:

- **Phase 1 (제품 MVP 안정화)**: 5주 MVP 결과물의 품질 보강, P1/P2 이월 항목 정리, 운영 문서화
- **Phase 2 (핵심 기능 고도화)**: FCM/APNs 운영 안정화, 청킹/임베딩 품질 개선, AI 카드 생성 품질 개선, 리더보드 확장
- **Phase 3 (고도화)**: MFA 정책 강화, Graph PageRank 고도화, RAG 품질 개선, 신고/모더레이션 운영 고도화
- **Phase 4 (분리 검토)**: 트래픽·소유 기준 모듈 → 서비스 추출

각 Phase의 범위·일정 상세는 `SYNAPSE_Service_Consolidation.md` §5 및 `09_Git_규칙_정의서` v2.0 §0.2 참조. 본 17의 실행 기준은 W1~W5이며, 장기 Phase는 5주 이후의 제품화 방향을 설명한다.
...
| v3.1 | 2026-05-15 | learning-card/learning-ai를 별도 서비스명이 아닌 synapse-learning-svc 내부 런타임/모듈로 표기 정합화 |
| v3.2 | 2026-05-15 | 학기 프로젝트 5주 MVP 구현·시연 범위와 제품 장기 Phase 로드맵을 분리해 PRD/TASK 실행 범위와 정합화 |
